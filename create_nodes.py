...
#settings:
SIZE = 100000 #note not all nodes will be created because of repeated values due to psuedorandom random function

parser = OptionParser()
parser.add_option("-n", "--nodes", dest="nodes",
                  help="number of nodes created within the system", default=1000)

(options, args) = parser.parse_args()

threads = int(options.nodes)
...
class Address(object):
	...
	def __hash__(self):
		self.hash = hash(("%s:%s" % (self.ip, self.port))) % SIZE
...
def main():
	parser = OptionParser()
	parser.add_option("-n", "--nodes", dest="nodes", help="number of nodes created within the system", default=1000)
	(options, args) = parser.parse_args()
