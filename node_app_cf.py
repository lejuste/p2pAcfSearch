...
def readCF(obj):
    obj.toStringObj = MethodType(cuckoo.toString, obj)
    return obj.toStringObj()

def setCF(obj,inputString):
    obj.fromStringObj = MethodType(cuckoo.fromString, obj)
    obj.fromStringObj(inputString)
...
def cuckItemExists(keyword):
    global cuckooDict
    for item in cuckooDict:
        if item == keyword:
            return True
    return False

def cuckItemAdd(keyword,fileName):
    global bloomDict

    # if cuck item exists, print
    if(cuckItemExists(keyword)):
        keyBloom = cuckooDict.get(keyword, [])
        print("adding cuck filter response: " + str(keyBloom.insert(fileName)))
    else:
        new_keyword_filter = CuckooFilter(capacity=10000, bucket_size=4, fingerprint_size=1)
        new_keyword_filter.insert(fileName)
        cuckooDict[keyword] = new_keyword_filter

def bloomItemCheck(keyword):
    if(bloomItemExists(keyword)):
        theFilter = cuckooDict.get(keyword, [])
        # print 'results:'
        # print theFilter.checkFilter(keyword)
        # print 'end of results'
        return theFilter.contains(keyword)
    else:
        return 0

def getKeywordFilter(keyword):
    return cuckooDict.get(keyword, 0)
